...
class ParseState {
  ...
  incrementId() {
    return new ParseState(this.input, this.at, this.next_id + 1);
  }
}
...
class ParseNode<T> {
  ...
  map<B>(f: (t: T) => B) {
    return new ParseNode(this.id, this.span, f(this.data));
  }
}

class ParseResult<T> {
  constructor(public state: ParseState, public node: ParseNode<T>) {}

  map<B>(f: (t: T) => B) {
    return new ParseResult(this.state, this.node.map(f));
  }
}

class Parser<T> {
  constructor(
    public parse: (state: ParseState) => undefined | ParseResult<T>
  ) {}

  static simple<T>(f: (input: string) => undefined | [number, T]) {
    return new Parser((state) => {
      const res = f(state.input.slice(state.at));
      if (res) {
        return new ParseResult(
          new ParseState(state.input, state.at + res[0], state.next_id + 1),
          new ParseNode(state.next_id, [state.at, state.at + res[0]], res[1])
        );
      }
    });
  }
  ...
  map<B>(f: (t: T) => B) {
    return new Parser<B>((state) => {
      return this.parse(state)?.map(f);
    });
  }
}

function tag(str: string) {
  return Parser.simple((input) => {
...
function seq<P1>(p1: P1): Parser<[P1]>;
function seq<P1, P2>(p1: P1, p2: P2): Parser<[P1, P2]>;
function seq<P1, P2, P3>(p1: P1, p2: P2, p3: P3): Parser<[P1, P2, P3]>;
function seq<P1, P2, P3, P4>(
  p1: P1,
  p2: P2,
  p3: P3,
  p4: P4
): Parser<[P1, P2, P3, P4]>;
function seq(...parsers: Parser<any>[]): Parser<any[]> {
  return new Parser((state) => {
    const begin = state.at;

    const nodes: any[] = [];
    for (const p of parsers) {
      const r = p.parse(state);
      if (!r) {
        return;
      }
    ...
    return new ParseResult(state, new ParseNode(id, [begin, state.at], nodes));
  });
}
...
Deno.test("sdf", () => {
  const quickParse = <T = any>(p: Parser<T>, input: string) => {
    return p.parse(new ParseState(input))?.node;
  };

  expect(quickParse(regex(/^abc/), "abcdef")).toMatchObject({
    data: "abc",
  });

  expect(quickParse(regex(/^abc/), "bcdef")).toBeUndefined();

  expect(quickParse(tag("abc"), "abcdef")).toMatchObject({
    data: "abc",
  });

  expect(quickParse(tag("abc"), "bcdef")).toBeUndefined();
});
...
//     move |input: I| {
//         if check(&input) { p.parse(input) } else { None }
//     }
// }
...
//         if check(&r) { Some((input, r)) } else { None }
//     }
// }
